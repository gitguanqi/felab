...
function getRandomPic(imgs) {  
  let index = Math.ceil(Math.random(0,1)*(imgs.length-1));
  return imgs[index];
}
...
function createImg (onload, imgLibs) {
  const img = new Image()
  img.crossOrigin = "Anonymous"
  img.onload = onload
  img.onerror = () => {
   img.setSrc(getRandomPic(imgLibs))
  }
...
class SlidingValidate {
  constructor ({ el, onSuccess, onFail, onRefresh, imgLibs }) {
  ...
  reset () {
    ...
    this.clean()
    this.img.setSrc(getRandomPic(this.imgLibs))
  }
}

window.SlidingValidate = {
  init: function (opts) {
    return new SlidingValidate(opts).init()
  }
}